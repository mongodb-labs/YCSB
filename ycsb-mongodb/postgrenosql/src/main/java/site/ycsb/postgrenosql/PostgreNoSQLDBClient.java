...
public class PostgreNoSQLDBClient extends DB {
  ...
  public Status update(String tableName, String key, Map<String, ByteIterator> values) {
    try{
      ...
      jsonbObject.setValue(fields.toJSONString());

      updateStatement.setObject(1, jsonbObject);
  ...
  public Status insert(String tableName, String key, Map<String, ByteIterator> values) {
    try{
      ...
      dataObject.put("YCSB_KEY", key);

      JSONObject fields = new JSONObject();
      for (Map.Entry<String, ByteIterator> entry : values.entrySet()) {
        fields.put(entry.getKey(), entry.getValue().toString());
      }
      dataObject.put("YCSB_VALUE", fields);

      PGobject jsonbObject = new PGobject();
      jsonbObject.setType("jsonb");
      jsonbObject.setValue(dataObject.toJSONString());

      insertStatement.setObject(1, jsonbObject);
  ...
  private String createReadStatement(StatementType readType) {
    StringBuilder read = new StringBuilder("SELECT data->>'" + PRIMARY_KEY + "' AS " + PRIMARY_KEY);

    if (readType.getFields() == null) {
      read.append(", (jsonb_each_text(data->'YCSB_VALUE')).*");
    } else {
      for (String field : readType.getFields()) {
        read.append(", data->'YCSB_VALUE'->>'" + field + "' AS " + field);
      }
    }
  ...
  private String createScanStatement(StatementType scanType) {
    StringBuilder scan = new StringBuilder("SELECT data->>'" + PRIMARY_KEY + "' AS " + PRIMARY_KEY);
    if (scanType.getFields() != null) {
      for (String field : scanType.getFields()) {
        scan.append(", data->'YCSB_VALUE'->>'" + field + "' AS " + field);
      }
    }
    scan.append(" FROM " + scanType.getTableName());
    scan.append(" WHERE data->>'");
    scan.append(PRIMARY_KEY);
    scan.append("' >= ?");
    scan.append(" ORDER BY data->>'");
    scan.append(PRIMARY_KEY);
    scan.append("' LIMIT ?");
  ...
  private String createUpdateStatement(StatementType updateType) {
    ...
    update.append(" SET data = jsonb_set(data, '{YCSB_VALUE}', (data->'YCSB_VALUE') || ?::jsonb, true)");
  ...
  /// Builds this string INSERT INTO usertable (data) VALUES (?::jsonb);
